...
|---|---|---|---|---|---|
| 3 | AI smart cart / upsell prediction | **Was silently broken, not "not started".** Staff-side `WaiterInterface.tsx` had a complete upsell banner UI wired to a working nightly-compute pipeline, but `useUpsellRules.ts` cast raw snake_case Supabase rows directly to a camelCase `UpsellRule` interface with no field mapping — every `rule.suggestedItemId` read `undefined`, so the banner never rendered in production, ever. | Small | High | **In progress 2026-07-10** — fixing the bug and extending to the QR customer cart in the same pass. Spec: `docs/superpowers/specs/2026-07-10-qr-menu-upsell-and-feedback-design.md`. |
| 6 | Digital post-payment feedback kiosk | **Mostly already built**, just not customer-discoverable. `TableFeedback.tsx` (313 lines) is a complete, working rating/comment form at `/feedback/:tenantSlug/:tableId`, but the only path to it was a staff member manually generating and sharing a link from `TableManagement.tsx`. Nothing in the QR menu itself pointed to it. | Small | Low-medium | **In progress 2026-07-10** — adding a self-service "Rate Your Visit" link in the QR menu footer. Auto-detecting bill closure to auto-transition the page (the fuller "kiosk" interpretation) was explicitly considered and deferred — see spec's Non-goals for why. |

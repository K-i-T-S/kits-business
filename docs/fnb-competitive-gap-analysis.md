...
> **Omega deep-dive update (2026-07-11):** a dedicated follow-up research pass (prompted by the founder specifically naming Omega as the primary competitive threat for client acquisition) went well beyond the surface-level check below. Full findings, sourced: market-share claim traced to Omega's own marketing copy with zero independent corroboration anywhere (no press, no analyst data) — do not repeat "78%" or "80%" as fact in any KiTS materials; "the dominant/most visible incumbent" is the defensible framing. Working hypothesis (circumstantial, not confirmed at the code level): Omega's core product is closer to legacy on-premise Windows POS with cloud features bolted on than to cloud-native SaaS — they sell branded touchscreen terminal hardware ($410) and kiosks ($1,950+) through a dedicated hardware store, every sales path is "request a demo"/quote-only (no self-serve signup), their own marketing hedges with "cloud or on-premise deployment," and one independent customer review unprompted called the system "old fashioned - not secure." No pricing is published anywhere. Real named clients (Casino du Liban, Peninsula Lebanon, plus Dunkin'/Burger King/PAUL/Chateau Ksara logos, moderate confidence) skew toward large enterprise/hospitality accounts, not the long tail of independents. **The clearest, evidence-backed displacement wedge**: multiple independent reviewers specifically and repeatedly flag Omega's multi-branch reliability breaking down — *"good for 1 branch... does not work for more than 1 branch... database keeps on crashing"* — plus consistently poor support ("untrained technicians," "transferred between people all the time"). Strategic read (inference, not sourced fact): the smart wedge for client acquisition is growing independent multi-location Lebanese restaurants currently on Omega who are hitting exactly the pain their own users describe — not a head-on assault on Omega's flagship enterprise accounts, where switching costs and integration depth are highest and least documented. Confirmed no Lebanese/MENA F&B vendor (Omega, BIM POS, Foodics, or newly-found competitor MAPOS) offers 3D floor-plan visualization — this remains genuine, open KiTS differentiation. **New competitive finding from this pass**: MAPOS (Lebanese, I-MAD Technology) markets "clock in/out with PIN codes" AND "role-based permissions" in their own copy — unverified (no live demo checked), but specific enough that it should be verified hands-on before finalizing any positioning that assumes staff-PIN-login/RBAC is clear KiTS whitespace against the whole Lebanese market, not just Omega/BIM POS. See `docs/superpowers/specs/2026-07-11-platform-roadmap-design.md` for how this feeds the product roadmap.

> **MAPOS PIN/RBAC claim verified (2026-07-12):** direct fetch of `mapos.tech`'s restaurant-POS blog page and homepage (not just search snippets) confirms MAPOS's own copy does state "Clock in/out with PIN codes" and "Role-based permissions" / "Role-based access control with unlimited users and advanced permissions" verbatim. **Do not position KiTS's PIN login + RBAC as unclaimed Lebanese-market whitespace — a prospect can check MAPOS's site and see the claim in writing.** That said, the claim is unconfirmable-in-practice: MAPOS has no self-serve signup, no free trial, no live interactive demo, and no product screenshots anywhere public (every CTA gates behind "Request Demo"/contact-sales); no granularity is disclosed on how roles/permissions actually work; and — unlike Omega, where independent reviews were findable both praising and criticizing the product — zero independent corroboration exists anywhere for MAPOS (absent from SoftwareSuggest's 22-vendor Lebanon POS listing, no Reddit/forum mentions, no third-party reviews found). MAPOS also reads as a broad multi-vertical ERP (retail/restaurants/construction/rentals/healthcare/spas), not an F&B specialist — its public testimonials are a medical center and a construction company. **Revised positioning**: retire "first with PIN + RBAC"; the still-standing, unclaimed differentiator is Track 2's actual build — genuinely different, purpose-built home screens per job function (not just permission-gating one shared screen), owner-customizable at the custom-role level — which no vendor found in either research pass (Omega, BIM POS, MAPOS) claims or demonstrates.

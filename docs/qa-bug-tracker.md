...
| BUG-004 | No granular per-employee activity/cash-handling audit trail (denominations, narrative log) | High | Missing Feature | Briefed | — |
...
| BUG-011 | Four components (`RolesAndPermissionsManager`, `TipsManagement`, `EnterpriseDashboard`, `UserProfileModal`) run independent stale `employees` fetches instead of `AppContext`'s already-correct shared state | Medium | UX/Flow | Briefed | — |
...
| BUG-025 | "Variant Intelligence" panel (attributes, cost-trend arrows, cost history) is fully decorative dead UI on every product for every tenant — no `product_variants` table was ever built, every product is synthesized into one fake empty variant | Medium | Cross-Vertical Conflict / Missing Feature | Briefed | — |
| BUG-026 | Plan product-count limits (`PLAN_LIMITS`, starter=50) are defined but never enforced anywhere — no warning, no block | Medium | Missing Feature | Briefed | — |
...
| BUG-030 | `PurchaseOrderManagement.tsx` runs its own independent `products`/`suppliers` fetch instead of `AppContext`'s shared `products` state | Medium | UX/Flow | Briefed | — |
...
| ID | Title | Severity | Category | Status | Files |
|---|---|---|---|---|---|
| BUG-032 | POS split-payment breakdown never persisted — any 2+-method sale is recorded as 100% a single method (`'cash'`), corrupting cash reconciliation and EOD reporting | High | Data Integrity | Briefed | — |
| BUG-033 | `addSale`'s local insert hardcodes `discount`/`tax_amount` to `0` — the `Sale` type never carried these fields, so the breakdown is permanently lost per sale (dormant today, nothing reads it back yet, but corrupts any future VAT/discount reporting built on this data) | High | Data Integrity | Briefed | — |
| BUG-034 | Loyalty-points redemption is a client-side read-then-write (race condition) — same class as the already-fixed BUG-031 | Medium | Data Integrity | Briefed | — |
| BUG-035 | **`POS.tsx` calls a non-existent `upsert_customer_points` RPC — loyalty points silently never accrue on any sale, for any tenant.** Independently confirmed live: no such function exists, and no trigger on `customer_points`/`point_transactions` exists either, contradicting `CLAUDE.md`'s migration-log claim of "earn/redeem triggers." | **Critical** | Functional Bug | Briefed | — |
| BUG-036 | CRM Segments/Communications/Analytics tabs in `Customers.tsx` are fed hardcoded empty arrays — their backing tables were never applied to the live schema (only exist in an archived migration file) | Medium | Missing Feature | Briefed | — |
...
| BUG-038 | Tip distribution records are `localStorage`-only — never written to the database, lost on browser data clear, invisible across devices/terminals | High | Missing Feature / Data Integrity | Briefed | — |
| BUG-039 | `restaurant_cash_sessions.denomination_breakdown` is 100% dead — folded into BUG-004 | High | Missing Feature | Briefed (see BUG-004) | — |
| BUG-040 | Zero employee attribution (`opened_by`/`created_by`) on any cash session or movement — folded into BUG-004 | High | Data Integrity | Briefed (see BUG-004) | — |
| BUG-041 | Cash-drawer reconciliation relies on manual re-entry, fully disconnected from actual `sales` records — folded into BUG-004 | High | Data Integrity | Briefed (see BUG-004) | — |
...
**Severity:** Medium · **Category:** Data Integrity · **Status:** Briefed
...
**Severity:** Critical · **Category:** Functional Bug · **Status:** Briefed
...
| BUG-053 | `complete_delivery_order` (DB function) hardcodes `discount=0, tax_amount=0` for every delivery sale — same bug class as BUG-033, now confirmed in a second independent code path | High | Data Integrity | Briefed | — |
| BUG-054 | Delivery sales have zero employee attribution (`sales.employee_id` never set), unlike dine-in sales | Medium | Data Integrity | Briefed | — |
| BUG-055 | `complete_delivery_order` has no idempotency guard — a double-click (or flaky-network retry) on "Mark Picked Up" can insert two `sales` rows for one delivery order | Medium-High | Data Integrity | Briefed | — |
...
Live comparison of the two order-finalization RPCs: `finalize_restaurant_order` (dine-in) computes and stores real `discount`/`tax_amount` from the order's actual percentages. `complete_delivery_order` hardcodes both to `0` regardless of the real numbers — Lebanese VAT (11%) is never recorded as a line item for any delivery sale, even though the aggregator-reported `total_usd` presumably already has it embedded, just not broken out. Same silent-data-loss pattern as BUG-033, now confirmed in a second, independent code path — a DB function, not just frontend.

**Agent Brief (paste-ready):**
> Fix `complete_delivery_order` (a live `SECURITY DEFINER` Postgres function — confirm current definition via `pg_get_functiondef` before editing, per established discipline) to record real `discount`/`tax_amount` instead of hardcoded zeros, mirroring what `finalize_restaurant_order` already does correctly. First confirm what breakdown data the aggregator webhook payloads (Toters/Talabat/Zomato/Careem, via the `delivery-webhook` Edge Function) actually provide — if they don't separate tax/discount from the total, this may need a reasonable VAT-inclusive estimate rather than exact figures; a founder/product decision, not purely mechanical. Bundle with BUG-054 and BUG-055 below — all three touch the same function.
...
`finalize_restaurant_order` sets `sales.employee_id` from the order's waiter. `complete_delivery_order` never sets it at all — every delivery sale is permanently unattributed to whichever staff member accepted/completed it, compounding the general cash/activity-attribution gap already in BUG-004/040.

**Agent Brief:** fold into the BUG-053 fix pass — set `sales.employee_id` from whichever employee called `complete_delivery_order` (their auth `user_id`, resolved to an `employees.id` the same way other attribution in this codebase works).
...
`finalize_restaurant_order` explicitly checks for an existing `sales` row before inserting (deliberately idempotent). `complete_delivery_order` has no equivalent check, and `DeliveryOrders.tsx`'s "Mark Picked Up" button has no disabled-during-request state and no optimistic status flip before the poll refetch — a fast double-click or a flaky-network retry can fire the RPC twice, inserting two `sales` rows for one delivery order and inflating reported revenue.

**Agent Brief:** fold into the BUG-053 fix pass — add the same "does a sale already exist for this order" guard `finalize_restaurant_order` already uses, and disable the "Mark Picked Up" button in `DeliveryOrders.tsx` while the request is in flight.
...
| BUG-062 | Branch-level menu availability/price overrides may be entirely unwired downstream — written and read only by two management screens, zero references found in the QR menu/POS/Waiter ordering paths (**unconfirmed**, needs a trace of `get_public_menu()`) | Medium-High | Missing Feature (unconfirmed) | Briefed | — |
| BUG-063 | `RestaurantSettings.tsx` and `SystemSettings.tsx` both write to the shared `tenants` table independently — latent last-write-wins risk if their field sets ever overlap (unconfirmed whether they currently do) | Low-Medium | Cross-Vertical Conflict (unconfirmed) | Briefed | — |
| BUG-065 | Three POS "business policy" settings (default payment method, require-customer-on-sale, auto-print receipt) save to `localStorage` only — `POS.tsx` never reads any of the three keys, so toggling them does nothing | High | Missing Feature | Briefed | — |
| BUG-066 | Tenant deletion ("Danger Zone") is an immediate, fully-cascading, irreversible delete of all business data, gated only by typing the business name on the same screen — no re-authentication, no grace period | High | Data Integrity / Safety Design | Briefed — **founder decision needed** | — |
...
| BUG-075 | `HelpSupport.tsx`'s "Submit Ticket" shows a fabricated success state regardless of whether the `mailto:` fallback actually reaches anyone (no default mail client = silent failure behind a fake "submitted" confirmation) | Medium | UX/Flow | Briefed | — |
| BUG-076 | `NarcoticsRegister.tsx`'s `pharmacist_name` is unverified free text, not bound to the authenticated employee — anyone can type any name into a legally-mandated controlled-substance log | High | Security/RBAC (Compliance) | Briefed | — |
...
| BUG-088 | `StockkeeperHomeHub.tsx`'s PO-receiving flow updates `restaurant_ingredients.current_stock` via read-then-write — same race-condition class already fixed twice this session via atomic RPCs, on a table that doesn't have one yet | Medium-High | Data Integrity | Briefed | — |
...
**Severity:** Medium · **Category:** Cross-Vertical Conflict · **Status:** Briefed

...
| BUG-045 | Preset-bundle `$0`-priced charge rows have no visual grouping/context on Kitchen Display — **unconfirmed, needs a live order test** | Low-Medium | UX/Flow | **Confirmed and fixed** (2026-07-14), via code trace rather than a live order | `src/pages/restaurant/KitchenDisplay.tsx` |
...
| BUG-051 | Events have zero link to `restaurant_tables`/sections — booking a room for a private event does nothing to block those tables from regular walk-in/reservation/waitlist seating | Medium | Missing Feature / Cross-Vertical Conflict | **Accepted as a deliberate limitation** (2026-07-14) — judgment call, not built; see entry | — |
...
**Severity:** Medium · **Category:** Missing Feature / Cross-Vertical Conflict · **Status:** Accepted as a deliberate limitation (2026-07-14)

**Judgment call, no code change:** the brief itself flagged this as a scope decision, not a mechanical fix, and offered accepting the current limitation as one of the two real options. Chose that over building table-linking because: (a) the real-world collision risk is genuinely lower than it sounds — events are pre-planned, staff are typically aware; (b) enforcement would mean modifying three separate, already-hardened atomic seating paths this session built/relies on (`fn_seat_reservation`, `fn_seat_waitlist_party`, `WaiterInterface`'s direct seating) for a benefit that's more theoretical than reported; (c) `room_section` isn't even linked to a consistent table/section taxonomy yet, so enforcement would need that groundwork first regardless. Revisit if a real double-booking incident is ever reported.
...
| BUG-061 | Two disconnected menu-item cost sources — `MenuManagement.tsx`'s manually-typed `cost_price_usd` vs. the real, now-accurate recipe-costing system (Batch 1 fixes) — nothing reconciles or shows them together | Medium | Data Integrity / Cross-Vertical Conflict | **Fixed** (2026-07-14) | `src/pages/restaurant/MenuManagement.tsx` |
...
**Severity:** Medium · **Category:** Data Integrity / Cross-Vertical Conflict · **Status:** Fixed (2026-07-14)

**Fix applied, judgment call on the open question:** rather than making recipe cost authoritative outright, `ItemFormModal` now shows both side by side when a menu item has a linked recipe (`restaurant_menu_item_recipes`) — the manually-typed `cost_price_usd` alongside the real cost from the already-fixed `get_recipe_cost()` RPC, with a divergence warning when they don't match. Chose this over hard-overriding the manual value because some items (a simple bottled drink, say) genuinely have no recipe and need a manual cost — auto-overriding would be wrong for those. No change when no recipe is linked.
...
| BUG-074 | `RestaurantAnalytics.tsx` has no staleness indicator for demand-forecast data — a silently-failed nightly cron job (a risk CLAUDE.md itself already flags) would show old data with no way to tell it's stale | Medium | Missing Feature | **Fixed** (2026-07-14) | `src/pages/restaurant/RestaurantAnalytics.tsx` |

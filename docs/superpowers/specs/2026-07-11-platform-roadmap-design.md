...
3. **1b-iv (not started):** custom-role enforcement — loosen the `custom_roles.base_role` CHECK constraint to all 8 canonical roles (founder-confirmed); fix `customRoleId` propagation through `send-invitation`/`accept_pending_invitation` (the real fix behind Tier 0.4's stopgap); wire `custom_roles.permissions` to actually gate something (currently stored, never read anywhere); re-enable `InviteTeamMemberModal`'s hidden custom-role picker (reverses `SHOW_CUSTOM_ROLES_IN_INVITE = false`) once this lands. `RoleGuard.tsx` (the second, unreconciled gating system found during research) still needs reconciling/retiring — deferred, not blocking.

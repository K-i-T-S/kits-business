...
1. **1b-i**: upgrade `coerceRole()` to return the full `RoleType` (8 roles) instead of collapsing to the legacy `UserRole` (4) — cascades to `SubscriptionContext`'s `role` state type and every consumer.
2. **1b-ii**: wire `canPerform`/`RoleGate` to consume the already-defined `ROLE_PERMISSIONS` (8-role table, currently unused) instead of legacy `ROLE_ACTIONS`; reconcile/retire `RoleGuard` (the second, unreconciled gating system found during research).
3. **1b-iii**: widen `RoleRoute`'s `allowedRoles` from `UserRole[]` to `RoleType[]` (small change, component logic unaffected), then wire it through `App.tsx`'s ~51 routes at the full 8-role granularity.
4. **1b-iv**: custom-role enforcement — loosen the `custom_roles.base_role` CHECK constraint to all 8 canonical roles; fix `customRoleId` propagation through `send-invitation`/`accept_pending_invitation` (the real fix behind Tier 0.4's stopgap); wire `custom_roles.permissions` to actually gate something (currently stored, never read anywhere); re-enable `InviteTeamMemberModal`'s hidden custom-role picker (reverses the `SHOW_CUSTOM_ROLES_IN_INVITE = false` stopgap) once this lands.
5. **1b-v**: pre-seed suggested starter custom roles per vertical — needs seed data + a UI entry point (onboarding industry-selection step, or a "suggested roles for your industry" action in `CustomRolesManager`).

Today's `RoleRoute` component (`src/components/RoleRoute.tsx`, built against `UserRole[]`) isn't wasted — 1b-iii is a type-widening pass on it once 1b-i/1b-ii land, not a rebuild.

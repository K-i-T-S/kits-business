...
2. **1b-iii: partially done.** `RoleRoute`'s `allowedRoles` widened from `UserRole[]` to `RoleType[]` (forced by 1b-i, not optional — `RoleType` isn't assignable to the narrower `UserRole`) — component logic unaffected, 5 tests passing including one proving an 8-role-only value (`supervisor`) now works. **Not yet done:** wiring `RoleRoute` through `App.tsx`'s ~51 routes — still real design work (which routes get which of the 8 roles), not started.

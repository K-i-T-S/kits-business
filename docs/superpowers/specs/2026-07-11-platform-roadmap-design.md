...
**1b (next up, not started):** wire `RoleRoute` through `App.tsx`'s ~51 route definitions — deciding which routes get which `allowedRoles` is real design work deserving its own session, not a mechanical follow-on to 1a.

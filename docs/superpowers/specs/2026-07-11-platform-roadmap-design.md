...
4. **1b-v (not started):** pre-seed suggested starter custom roles per vertical. Founder-confirmed restaurant starter set: **Receptionist** (base `cashier`, minus `make_sales`), **Waiter** (base `cashier`, as-is — `WaiterInterface.tsx` already gates real actions behind `make_sales`), **Head Chef** (base `supervisor`, plus `manage_products`), **Sous Chef** (base `stockkeeper`, as-is), **Argile Staff** (base `cashier`, as-is — tobacco refills are billable events). Needs seed data + a UI entry point (onboarding industry-selection step, or a "suggested roles for your industry" action in `CustomRolesManager`).

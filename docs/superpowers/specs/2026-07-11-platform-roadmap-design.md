...
**Found while researching this (Track 1d note):** `employees.user_id` already exists as a column and is already correctly populated by `accept_pending_invitation()` — the employees↔tenant_users reconciliation problem is already half-solved for invite-created employees. The other creation path (`AppContext.addEmployee`, the "Add Teammate" quick-add button) does *not* set `user_id` — still genuinely disconnected. Track 1c's new PIN-employee-creation edge function should set `user_id` correctly from the start, and the "Add Teammate" path should eventually be reconciled or retired in favor of it — not resolved today, flagged for Track 1d.

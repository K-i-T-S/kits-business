...
export function ActionQueueWidget({ title, icon, accent, items, emptyLabel, loading }: ActionQueueWidgetProps) {
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-slate-900 p-4 flex flex-col gap-3">
          ...
          {items.map((item) => (
            <motion.div
              ...
              className={`flex items-center gap-3 rounded-xl border px-3 py-2.5 ${
                item.urgent ? 'border-red-500/25 bg-red-500/[0.06]' : 'border-white/10 bg-white/5'
              }`}
            >

...
interface RateLimitConfig {
  ...
  keyGenerator?: (request: RequestLike) => string;
  onLimitReached?: (request: RequestLike, response: ResponseLike) => void;
}
...
export class RateLimiter {
  ...
  private getKey(request: RequestLike): string {
    ...
    return `${String(ip)}:${String(userAgent)}`;
  }
  ...
  public middleware = (request: RequestLike, response: ResponseLike, next?: (err?: unknown) => void) => {
...
export const createApiRateLimiter = () => new RateLimiter({
  ...
  keyGenerator: (request: RequestLike) => {
    ...
    return userId ? `user:${userId}` : `ip:${String(ip)}`;
  },
});
...
export const createAuthRateLimiter = () => new RateLimiter({
  ...
  keyGenerator: (request: RequestLike) => {
    ...
    return `auth:${String(ip)}:${email}`;
  },
  onLimitReached: (request: RequestLike, _response: ResponseLike) => {
    console.warn(`Rate limit exceeded for auth attempt from IP: ${String(request.headers?.['x-forwarded-for'])}`);
  },
});
...
export const createUploadRateLimiter = () => new RateLimiter({
  ...
  keyGenerator: (request: RequestLike) => {
    const userId = request.user?.id;
    return userId ? `upload:user:${userId}` : `upload:ip:${String(request.connection?.remoteAddress)}`;
  },
});

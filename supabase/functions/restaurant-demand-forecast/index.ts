...
// Matches the actual restaurant_demand_forecasts columns (migration
// 20260622_000042_restaurant_ai.sql) — narrower than DayForecast, which
// carries the full internal computation. Seasonality/holiday/Ramadan/
// summer-peak detail has no dedicated column and is packed into `factors`.
...
/**
 * Convert a Date to a "zoned" Date whose UTC-based getters/setters reflect
 * the wall-clock time in `timeZone`. Matches date-fns-tz's toZonedTime
 * contract using only native Intl (no external package — the previous
 * date-fns-tz dependency failed to boot on Supabase's Edge Runtime due to
 * an esm.sh peer-resolution issue). Correct only in a runtime whose local
 * timezone is UTC, which Deno Edge Functions are.
 */
...
function toBeirutDate(date: Date): string {
  const zoned = toZonedTime(date, BEIRUT_TZ);
...
async function runForecastForTenant(
  ...
  // Build insert payload — mapped to the actual table columns (see ForecastRow)
  ...
  // Upsert: overwrite any forecasts for the same (tenant_id, date)
